...
- [ ] 连接配置中填写 SSH 参数后，能通过 SSH 隧道连接到远程数据库
- [ ] 密码认证和密钥认证两种方式都支持
- [ ] 连接断开后 SSH 隧道正确关闭
- [ ] SSH 连接失败的错误信息友好易懂
- [ ] 支持 MySQL 和 PostgreSQL 通过 SSH 隧道连接
...
- [ ] 连接树表右键菜单 "复制表 → 仅结构" 正常工作
- [ ] 连接树表右键菜单 "复制表 → 结构和数据" 正常工作
- [ ] 复制后的表结构与原表一致（索引、约束需视数据库支持情况）
- [ ] 复制数据后行数正确
...
- [ ] 表右键菜单 "转储 SQL 文件" 打开导出对话框
- [ ] "仅结构" 模式生成正确的 CREATE TABLE DDL
- [ ] "结构和数据" 模式同时生成 DDL 和 INSERT 语句
- [ ] 生成的 SQL 文件可以直接在目标数据库执行
- [ ] 大表导出不卡 UI
...
- [ ] 导入向导支持 .xlsx 和 .xls 格式
- [ ] 多 Sheet 文件可选择目标 Sheet
- [ ] Excel 数据正确映射到表列
- [ ] 大文件导入不卡 UI（使用 Web Worker 可选）
